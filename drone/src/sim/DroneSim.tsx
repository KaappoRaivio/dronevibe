...
const Drone = () => {
	...
	useFrame((_, dt) => {
		...
		motorOffsets.forEach((offset, i) => {
			...
			body.current.applyImpulseAtPoint({ x: 0, y: thrusts[i], z: 0 }, worldPoint, true);
		});
	});

	return (
		<>
			<RigidBody ref={body} colliders={false} restitution={0.3} position={[0, 0.5, 0]}>
				<CuboidCollider mass={1} args={[0.5, 0.05, 0.5]} />
