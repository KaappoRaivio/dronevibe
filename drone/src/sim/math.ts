...
export class Mat3 {
	...
	static fromQuaternion({ x, y, z, w }: { x: number; y: number; z: number; w: number }) {
		const a = w;
		const b = x;
		const c = y;
		const d = z;

		return new Mat3(a * a + b * b - c * c - d * d, 2 * b * c - 2 * a * d, 2 * b * d + 2 * a * c, 2 * b * c + 2 * a * d, a * a - b * b + c * c - d * d, 2 * c * d - 2 * a * b, 2 * b * d - 2 * a * c, 2 * c * d + 2 * a * b, a * a - b * b - c * c + d * d);
	}
	...
	constructor(x1: number, x2: number, x3: number, y1: number, y2: number, y3: number, z1: number, z2: number, z3: number) {
		this.x1 = x1;
		this.x2 = x2;
		this.x3 = x3;
		this.y1 = y1;
	...
	scale(other: number): Mat3 {
		return new Mat3(this.x1 * other, this.x2 * other, this.x3 * other, this.y1 * other, this.y2 * other, this.y3 * other, this.z1 * other, this.z2 * other, this.z3 * other);
	}

	transpose(): Mat3 {
		return new Mat3(this.x1, this.y1, this.z1, this.x2, this.y2, this.z2, this.x3, this.y3, this.z3);
	}
	...
	add(other: Mat3): Mat3 {
		return new Mat3(this.x1 + other.x1, this.x2 + other.x2, this.x3 + other.x3, this.y1 + other.y1, this.y2 + other.y2, this.y3 + other.y3, this.z1 + other.z1, this.z2 + other.z2, this.z3 + other.z3);
	}
